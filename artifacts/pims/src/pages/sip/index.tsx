...
export default function SipPlanner() {
  ...
  const recordSip = useRecordSipExecution({
    mutation: {
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: getGetSipHistoryQueryKey() });
      },
    },
  });
  ...
  const allocTotal = (cat: Category) =>
    allocsFor(cat).reduce((s, a) => s + (Number(a.percent) || 0), 0);
  const categoryPercentFor = (cat: Category) =>
    cat === "equity_fund"
      ? equityPercent
      : cat === "debt_fund"
        ? debtPercent
        : metalsPercent;
  const allocValid = (cat: Category) => {
    const t = allocTotal(cat);
    return (
      allocsFor(cat).length === 0 ||
      Math.abs(t - categoryPercentFor(cat)) < 0.01
    );
  };
  ...
  const addAlloc = (cat: Category) => {
    const available = (assets || []).filter(
      (a) =>
        a.category === cat &&
        !allocs.find((al) => al.assetId === a.id && al.category === cat),
    );
    if (!available.length) return;
    const first = available[0];
    setAllocs((prev) => [
      ...prev,
      { assetId: first.id, assetName: first.name, category: cat, percent: 0 },
    ]);
  };
  ...
  const removeAlloc = (cat: Category, idx: number) => {
    ...
    setAllocs((prev) =>
      prev.filter((a) => !(a.assetId === target.assetId && a.category === cat)),
    );
  };
  ...
  const handleSave = () => {
    if (!categoryValid) {
      toast({
        title: "Category percentages must sum to 100%",
        variant: "destructive",
      });
      return;
    }
    if (!allAllocsValid) {
      toast({
        title: "Asset allocations must match each category percentage",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleExecuteSip = async () => {
    if (!categoryValid) {
      toast({
        title: "Fix category percentages first",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      await new Promise<void>((resolve, reject) => {
        recordSip.mutate(
          { data: { month: execMonth, totalAmount: monthlyAmount, breakdown } },
          {
            onSuccess: () => resolve(),
            onError: reject,
          },
      ...
      toast({
        title: "SIP Executed!",
        description: `${breakdown.length} transactions created for ${execMonth}.`,
      });
      ...
      toast({
        ...
        description:
          err?.data?.error || "Some transactions may have been created.",
        variant: "destructive",
      });
    } finally {
      setExecuting(false);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Configure and execute your systematic investment plan.
          </p>
        </div>
        <Button
          variant="default"
        ...
        <TabsContent value="planner" className="space-y-6 mt-4">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Total amount you invest each month via SIP.
              </CardDescription>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Split across asset categories — must sum to 100%.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {!categoryValid && (
                <Alert variant="destructive">
                  ...
                  <AlertDescription>
                    Percentages sum to <strong>{categoryTotal}%</strong>. Must
                    equal 100%.
                  </AlertDescription>
                </Alert>
              )}
                ...
                {[
                  {
                    label: "Equity Funds",
                    value: equityPercent,
                    set: setEquityPercent,
                    color: "text-blue-500",
                    amount: monthlyEquity,
                  },
                  {
                    label: "Debt Funds",
                    value: debtPercent,
                  ...
                  },
                  {
                    label: "Metals (Gold/Silver)",
                    value: metalsPercent,
                    set: setMetalsPercent,
                    color: "text-yellow-500",
                    amount: monthlyMetals,
                  },
                  {
                    label: "Opportunity Fund",
                    value: opportunityPercent,
                    set: setOpportunityPercent,
              ...
              <div
                className={`text-sm font-medium text-right pt-2 border-t ${categoryValid ? "text-green-500" : "text-red-500"}`}
              >
                Total: {categoryTotal}%{" "}
                {categoryValid
                  ? "✓"
                  : `(need ${(100 - categoryTotal).toFixed(1)}% more)`}
              </div>
            </CardContent>
          </Card>
          ...
          {(["equity_fund", "debt_fund", "metal"] as Category[]).map((cat) => {
            ...
            const catPercent = categoryPercentFor(cat);
                ...
                <CardHeader>
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <CardDescription>
                        Enter percentages of total SIP for individual{" "}
                        {label.toLowerCase()}. Sum must equal {catPercent}%.
                      </CardDescription>
                    </div>
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => addAlloc(cat)}
                      disabled={catAssets.length === catAllocs.length}
                    >
                ...
                <CardContent className="space-y-3">
                  {!valid && catAllocs.length > 0 && (
                    <Alert variant="destructive">
                      <AlertCircle className="h-4 w-4" />
                      <AlertDescription>
                        Allocations sum to <strong>{total}%</strong>. Must equal{" "}
                  ...
                  {catAllocs.length === 0 ? (
                    <div className="p-6 text-center text-muted-foreground border rounded-lg bg-muted/10 text-sm">
                      No sub-allocations set. Add funds above and assign
                      percentages from total SIP; this category should add up to{" "}
                      {catPercent}%.
                    </div>
                  ) : (
                    catAllocs.map((alloc, idx) => {
                      const assetAmount = (monthlyAmount * alloc.percent) / 100;
                      return (
                        <div
                          key={idx}
                        ...
                        >
                          <div className="flex-1">
                            <Select
                              value={String(alloc.assetId)}
                              onValueChange={(v) =>
                                updateAllocAsset(cat, idx, Number(v))
                              }
                            >
                              <SelectTrigger>
                          ...
                          <div className="flex items-center gap-1 w-28">
                            <Input
                              ...
                              value={alloc.percent}
                              onChange={(e) =>
                                updateAllocPercent(cat, idx, e.target.value)
                              }
                              min={0}
                              max={100}
                          ...
                          <div className="w-28 text-right text-sm text-muted-foreground shrink-0">
                            {formatCurrency(assetAmount)}
                          </div>
                          <Button
                            variant="ghost"
                            size="icon"
                            onClick={() => removeAlloc(cat, idx)}
                          >
                  ...
                  {catAllocs.length > 0 && (
                    <div
                      className={`text-sm font-medium text-right pt-2 border-t ${valid ? "text-green-500" : "text-red-500"}`}
                    >
                      Total: {total}%{" "}
                      {valid
                        ? "✓"
                        : `(target ${catPercent}%, ${total < catPercent ? `need ${(catPercent - total).toFixed(1)}% more` : `over by ${(total - catPercent).toFixed(1)}%`})`}
                    </div>
                  )}
                </CardContent>
              </Card>
        ...
        <TabsContent value="history" className="mt-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Past SIP executions and their breakdowns.
              </CardDescription>
            </CardHeader>
            <CardContent>
              {history && history.length > 0 ? (
                      ...
                      <div className="flex justify-between items-center p-4 bg-muted/20">
                        <div>
                          ...
                          <div className="text-xs text-muted-foreground">
                            Executed{" "}
                            {new Date(entry.executedAt).toLocaleDateString()}
                          </div>
                        </div>
                        <div className="text-right">
                          ...
                          <div className="text-xs text-muted-foreground">
                            {(entry.breakdown || []).length} allocations
                          </div>
                        </div>
                      </div>
                ...
                <div className="p-10 text-center border rounded-lg bg-muted/10">
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    Click "Execute SIP" to run your first SIP and it will appear
                    here.
                  </p>
                </div>
              )}
      ...
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              This will create SIP transactions for all configured asset
              allocations and record the execution in history.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-2">
            ...
            {!categoryValid && (
              <Alert variant="destructive">
                <AlertCircle className="h-4 w-4" />
                <AlertDescription>
                  Fix category percentages before executing.
                </AlertDescription>
              </Alert>
            )}
          </div>
